...
public class DataAndOperations extends ReadCSVFile {
    ...
    public Dataset<Row> jobsForCompany()
    {
        this.cleanDataFrame.createOrReplaceTempView("Temp_Table");
        final Dataset<Row> jobsForCompany = sparkSession.sql("select Company, count(Title) as NumberOfJobs from Temp_Table group by Company order by count(Title) desc");
        return jobsForCompany;
    }

    public Dataset<Row> MostPopularJobs()
    {
        this.cleanDataFrame.createOrReplaceTempView("Temp_Table");
    ...
    {
        this.cleanDataFrame.createOrReplaceTempView("Temp_Table");
        final Dataset<Row> MostPopularAreas = sparkSession.sql("select Country, count(*) as NumberOfAreas  from Temp_Table group by Country order by NumberOfAreas desc");
        return MostPopularAreas;
    }

    public void TopTenDemandingCompaniesPieChart(String AbsolutePathWithJPGName)
    {
        ...
        try {
            BitmapEncoder.saveJPGWithQuality (chart, AbsolutePathWithJPGName, 0.99f);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public void TopTenMostPopularJopsBarChart(String AbsolutePathWithJPGName)
    {
    ...
    public void TopTenMostPopularAreasBarChart(String AbsolutePathWithJPGName)
    {
    ...
    public String printSkillsOneByOne()
    {
        this.cleanDataFrame.createOrReplaceTempView("Temp_Table");
        ...
        String SkillsOneByOne = "";
        for (Map.Entry<String, Long> entry : sorted)
        {
            SkillsOneByOne =  SkillsOneByOne.concat(entry.getKey() + " : " + entry.getValue());
            SkillsOneByOne =  SkillsOneByOne.concat("<br>");
        }
